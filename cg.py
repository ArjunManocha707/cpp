...
def predict():
    if request.method == 'POST':
        ...
        # Instead of using the global model, load the model inside the predict route
        ...
        single_prediction_rf = model.predict(car_today_data_encoded)[0]
        print(f"Predicted Car Price: {single_prediction_rf:,.2f}")
        ...
        return render_template('result.html', prediction_rf=single_prediction_rf)
